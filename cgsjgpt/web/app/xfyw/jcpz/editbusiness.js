...
function init() {
    ...
    jQuery.ajax({
        ...
        type: "get",
        contentType: "application/json",
        url: "/api/XfywJcpz/BusinessBaseGet",
        headers: {
            'Authorization': window.sessionStorage.getItem('token')
        },
        data: {
        ...
        success: function (res) {
            ...
            data.ywlx = b.YWLX;
            data.ywyy = b.YWYY;
            data.yhphmisnull = b.YHPHMISNULL;
            data.cylshisnull = b.CYLSHISNULL;
            data.blms = JSON.parse(b.BLMS);
            data.shms = b.SHMS;
            data.temps = data.brzls = JSON.parse(b.BRZL);
            data.dlrzls = JSON.parse(b.DLRZL);
            data.dwzls = JSON.parse(b.DWZL);
        },
        error: function (res) {
            var ret = JSON.parse(res.responseText)
...
function save() {
    ...
    data.brzls.forEach(function (item) {
        updata.brzls.push(Object.assign({}, { ID: item.ID, CODE: item.CODE, NAME: item.NAME, bx: item.bx }));
    });
    if (data.blms.indexOf("代理个人业务") > 0) {
        data.dlrzls.forEach(function (item) {
            updata.dlrzls.push(Object.assign({}, { ID: item.ID, CODE: item.CODE, NAME: item.NAME, bx: item.bx }));
        });
    }
    if (data.blms.indexOf("代理单位业务") > 0) {
        data.dwzls.forEach(function (item) {
            updata.dwzls.push(Object.assign({}, { ID: item.ID, CODE: item.CODE, NAME: item.NAME, bx: item.bx }));
        });
    }
    ...
    jQuery.ajax({
        ...
        type: "post",
        contentType: "application/json",
        url: "/api/XfywJcpz/BusinessBaseEdit",
        headers: {
            'Authorization': window.sessionStorage.getItem('token')
        },
