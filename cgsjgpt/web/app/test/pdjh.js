    ...
    methods: {
        ...
        btnbj: function btnbj() {
            /*layer.prompt({
                ...
                end(data.currxh, data.curropid, data.currpdxxjhqkxh, "办结", "办结", value, data.second);
                layer.close(index);
            });*/
            var html = "<span id='bjinfo'>"
                + "<span style='display:block;margin:10px;'>业务系统流水号:</span>"
                + "<span style='display:block;margin:10px;'><input name='lsh' type='text' class='forminput inputstyle' placeholder='例：2190717033049' v-model='test' style='width:90%'></span>"
            ...
            layer.open({
                ...
                yes: function (index) {
                    ...
                    let msg = "";
                    if (!(lsh.length == 13 || lsh.length == 0 || lsh === undefined || lsh === '')) msg += "必须输入13位流水号<br/>";
                    if (sfzzpz == undefined || sfzzpz == null || sfzzpz == "") {
                        msg += "请选择是否制作牌证";
                    }
                    if (msg.length > 0) {
                        layer.alert(msg, {
