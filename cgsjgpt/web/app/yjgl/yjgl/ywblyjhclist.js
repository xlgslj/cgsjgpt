...
layui.config({
    ...
    table.render({
        ...
        , id: "table"
        , height: "full-85"
        , url: '/api/YjglYjgl/YwblYjhcList' //数据接口
    ...
    table.on('tool(test)', function (obj) {
        if (obj.event == "bl") {
            ...
            window.parent.layindex2 = window.parent.layer.open({
                title: "预警核查",
                type: 2,
                area: [parent.window.innerWidth >= 1600 ? "1600px" : "90%", parent.window.innerHeight >= 900 ? "900px" : "90%"],
                fixed: false, //不固定
                maxmin: true,
                content: ["app/yjgl/yjgl/ywblyjhc.html?id=" + obj.data.ID, (parent.window.innerWidth >= 1600 && parent.window.innerHeight >= 900 ? "no" : "yes")]
            });
        }
    });
