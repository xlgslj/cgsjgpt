...
var vm = new Vue({
    ...
    watch: {
        ...
        'notice.HPZLNO': function (nv, ov) {
            this.notice.HPZL = this.code.hpzls.filter(function (item) {
                return item.CODE1 == nv;
            })[0].NAME;
        }
    },
    methods: {
        saveandprint: function saveandprint() {
            ...
            if (this.notice.HPHM.length < 7) msg += "号牌号码输入错误!<br>";
            if (this.notice.TBLX == "") msg += "退办类型必选<br>";
            if (msg.length > 0) {
...
function init() {
    ...
    jQuery.ajax({
        ...
        success: function success(res) {
            ...
            data.code.ywlxs = res.jdcywlx;
            data.code.hpzls = res.hpzl;
            layer.close(index);
        },
