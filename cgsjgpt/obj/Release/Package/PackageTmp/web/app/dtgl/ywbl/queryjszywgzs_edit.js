...
layui.use(['laydate', 'layer', 'table', 'jquery'], function () {
    ...
    table.render({
        ...
        id: "table",
        height: "full-85",
        url: '/api/DtglYwbl/QueryJszywGzs' //数据接口
        ,
        headers: {
        ...
        parseData: function parseData(res) {
            ...
            res.data.forEach(function (item) {
                ...
                files.forEach(function (it) {
                    ...
                    if (it.CONTENT != "" && it.CONTENT != undefined) {
                        qsx.push("其他:" + it.CONTENT);
                    }
                });
                Object.assign(item, {
                    FILES: qsx.join("，")
    ...
    table.on('tool(test)', function (obj) {
        if (obj.event == "show") {
            window.parent.layindex2 = window.parent.layer.open({
                ...
                //不固定
                maxmin: true,
                content: ["app/dtgl/ywbl/queryjszywgzs-more-edit.html?id=" + obj.data.ID, (parent.window.innerWidth >= 1000 && parent.window.innerHeight >= 800 ? "no" : "yes")]
            });
        }
    });
});
