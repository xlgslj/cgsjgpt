...
function init() {
    ...
    var index1 = layer.load(0, { shade: [0.1, "#FFFFFF"] });
    jQuery.ajax({
        async: true,
        type: "get",
        contentType: "application/json",
        url: "/api/HmdglWztc/getsingle",
        headers: {
            'Authorization': window.sessionStorage.getItem('token')
        },
        data: {
        ...
        success: function (res) {
            ...
            if (res.state === 1) {
                ...
                data.hphm = d.HPHM
                data.hpzlno = d.HPZL
                data.hpzlname = d.HPZLNAME
                data.jdcsyr = d.JDCSYR
                data.gxr = d.GXR
                data.yxqlx = d.JSSJ === "9999-12-31" ? '永久' : '固定'
                data.jssj = d.JSSJ
                data.memo = d.MEMO
            }
        },
        error: function (res) {
            ...
            else { alert('错误:' + ret.Message) } try { layer.close(index1); } catch (e) { }
            return;
        }
    });
}
